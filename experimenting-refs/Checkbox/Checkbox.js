...
const setCheckedHandler = (checkbox, onChange, setChecked, isChecked) => {
  ...
  if (
    (typeof onChange === 'function' && onChange({ target: checkbox.current })) ||
...
const Checkbox = forwardRef(({ name, onChange = null, checked = false }, ref) => {
  ...
  const classes = [styles.checkboxEl, isChecked ? styles.checked : ''].join(' ').trim()
  ...
  return (
    <div
      className={classes}
      onClick={() => setCheckedHandler(checkbox, onChange, setChecked, isChecked)}
    >
      <div />
