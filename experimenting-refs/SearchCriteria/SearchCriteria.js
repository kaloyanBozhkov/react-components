...
const onChangeHandler = (inputName, checked, currentlyChecked, setChecked, alwaysOneChecked) => {
  ...
  if (checked) {
    ...
    return true
  } else if (currentlyChecked === inputName && !alwaysOneChecked) {
    //if checkbox is not checked, and is currently the checked one in state, uncheck it from state ((as long as always one checked is not stated))
    setChecked(null)
 ...
 */
const SearchCriteria = forwardRef(
  (
    { label = 'Some Label', type = 'text', defaultChecked = null, alwaysOneChecked = false },
    ref
  ) => {
        ...
        <div className={styles.searchOptions}>
          {checkboxesForType[type].map(({ name, label }) => (
            ...
            <Input
              ...
              onChange={({ target }) =>
                onChangeHandler(name, target.checked, checked, setChecked, alwaysOneChecked)
              }
            />
          ))}
        </div>
